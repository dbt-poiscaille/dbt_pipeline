...
from
 ...
 --where sale_id = '62c2b2b63e09a34364d74cbc'
) , 
...
select 
   ...
   round(sum(offerings_value_price_ttc),2) as bonus_revenue, 
  from bonus_data
  group by 1
) , 
...
select
    ...
    round(sum( case when sale_type ='order' then price_ttc end ),2) as order_revenue_ttc, 
    round(sum( case when sale_type ='locker' then price_ttc end ),2) as locker_revenue_ttc,
