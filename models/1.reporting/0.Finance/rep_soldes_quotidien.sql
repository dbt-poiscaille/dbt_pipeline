...
with
    daily_balance_transactions as (
        select
            ...
            case
                when type = 'payout'
                then cast(timestamp_seconds(available_on) as date)
            ...
            sum(  -- net = amount - fee
                case when type in ('charge', 'payment') then net else 0 end
            ) as payments,
            sum(
                case
                    when type in ('payment_refund', 'refund', 'payment_failure_refund')
                    then net
                    else 0
                end
            ) as refunds,
            ...
            sum(
                case
                    when
                        type = 'adjustment'
                        and lower(description) like 'chargeback withdrawal%'
                    then net
                    else 0
                end
            ) as chargeback_withdrawals,
            sum(
                case
                    when
                        type = 'adjustment'
                        and lower(description) like 'chargeback reversal%'
                    then net
                    else 0
                end
            ) as chargeback_reversals,
            sum(
                case
                    when
                        type = 'adjustment'
                        and lower(description) not like 'chargeback withdrawal%'
                        and lower(description) not like 'chargeback reversal%'
                    then net
                    else 0
                end
            ) as other_adjustments,
            sum(
                case
                    when
                        type not in (
...
select
    ...
    sum(daily_balance) over (partition by currency order by day)
    / 100.0 as current_balance,
