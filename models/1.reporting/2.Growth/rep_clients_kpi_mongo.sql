...
sale_data AS (
     SELECT 
        ...
        round(sum(offerings_value_price_ttc)/100,2) as monetary,
        ...
        round(sum(offerings_value_price_ttc)/100,2) as customer_revenue, 
        max(sale_date) as last_payment ,  
        round(sum(case when type_sale = 'abonnement' then offerings_value_price_ttc end )/100,2) as total_subscriptions,
        round(sum(case when type_sale = 'shop' then offerings_value_price_ttc end )/100,2) as total_shop,
        round(sum(case when type_sale = 'Petit plus' then offerings_value_price_ttc end)/100,2) as total_petitplus,
        round(sum(case when EXTRACT(YEAR FROM sale_date)  = EXTRACT(YEAR FROM CAST(CURRENT_DATE() AS DATE)) then offerings_value_price_ttc end )/100,2) as total_year,
        round(sum(case when EXTRACT(YEAR FROM sale_date)  = EXTRACT(YEAR FROM CAST(CURRENT_DATE() AS DATE))-1 then offerings_value_price_ttc end )/100,2) as total_last_year,
